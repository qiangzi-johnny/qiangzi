...
import com.krund.hotel.manage.pay.util.WebUtils;
...
public class WechatClient {
    ...
    public WechatUnifiedOrder.Response unifiedOrder(WechatUnifiedOrder unifiedOrder){
        unifiedOrder.setAppid(this.app_id); //公众号(开发者)id，必须
        unifiedOrder.setMch_id(this.mch_id); //商户id，必须
        unifiedOrder.setTrade_type(this.trade_type); //
        unifiedOrder.setNotify_url(WechatConfigs.getNotifyPay()); 
        ...
        try {
            ...
            String resultXml = WebUtils.post(WechatConfigs.getUnifiedorderUrl(),inputStreamEntity);
    ...
    public WechatRefund.Response refund(WechatRefund wechatRefund){
        ...
        try {
            InputStreamEntity inputStreamEntity = new InputStreamEntity(
                    new ByteArrayInputStream(requestXml.getBytes("UTF-8")), ContentType.APPLICATION_XML);
            String resultXml = WebUtils.post(WechatConfigs.getCertPath(), WechatConfigs.getMchId(), WechatConfigs.getRefundUrl(),inputStreamEntity);
    ...
    public WechatRefundQuery.Response refundQuery(WechatRefundQuery wechatRefundQuery){
        ...
        try {
            InputStreamEntity inputStreamEntity = new InputStreamEntity(
                    new ByteArrayInputStream(requestXml.getBytes("UTF-8")), ContentType.APPLICATION_XML);
            String resultXml = WebUtils.post(WechatConfigs.getRefundQuery(),inputStreamEntity);
    ...
    public String sign(TreeMap<String, ?> param){
        String paramUrl = SignUtil.joinKeyValue(new TreeMap<>(param),null,"&key="+this.app_secret,"&",true,"sign_type","sign","paySign");
